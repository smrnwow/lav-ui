<template lang="html">
  <label class="search-input" ref="wrap">
    <div class="search-input__wrap" v-show="selected.length">
      <span class="search-input__selected" v-for="(item, i) in selected" @click="removeSelected(i)" :key="item.val">
        {{ item.name }}
    ...
    <div class="" :style="{ position: 'relative' }">
      ...
      <transition name="fade">
        <div class="search-input__dropdown" v-show="dropdownVisible" ref="dropdown">
          <div v-for="(option, i) in getOptions" class="search-input__dropdown-item" @click="select(option)"
          :class="{
            'search-input__dropdown-item_active': getActive(option),
            'search-input__dropdown-item_hovered': i === iterator
          }" :key="option.value">
          {{ option.name }}
        </div>
...
export default {
  ...
  methods: {
    ...
    keyBoardSelect() {
      this.select(this.getOptions[this.iterator]);
    },
    clearInput() {
      this.$emit('input', '');
      this.searchString = '';
    ...
    scrollDropdown(e) {
      this.$nextTick(() => {
        ...
        if(e.keyCode === 40 && this.iterator < this.getOptions.length) {
          this.iterator++;
          activeItem = list.children[this.iterator];
        ...
        } else if(e.keyCode === 38 && this.iterator > 0) {
          this.iterator--;
          activeItem = list.children[this.iterator];
